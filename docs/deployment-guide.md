...
fn update context compartment-id ocid1.compartment.oc1..aaaaaaaavp2ks4ff7nk3vgm2i6il7sxnh4by474iyeuqsc57z4sogx7nj5sa
```
...
```bash
# Get function ID
FUNCTION_ID=$(oci fn function list --application-id ocid1.fnapp.oc1.iad.amaaaaaawe6j4fqacp4fg46c2xk4zyxv7nctku422mui3lg7yhhhonwscrlq --query 'data[0].id' --raw-output)

# Set environment variables
oci fn function update --function-id $FUNCTION_ID --config OCI_COMPARTMENT_ID=ocid1.compartment.oc1..aaaaaaaavp2ks4ff7nk3vgm2i6il7sxnh4by474iyeuqsc57z4sogx7nj5sa
oci fn function update --function-id $FUNCTION_ID --config OCI_OS_NAMESPACE=orasenatdpltintegration03
oci fn function update --function-id $FUNCTION_ID --config OCI_OS_BUCKET=delivery_image_repo
...
docker build -t iad.ocir.io/orasenatdpltintegration03/delivery-agent:latest .

# Login to OCIR
docker login iad.ocir.io

# Push the image
docker push iad.ocir.io/orasenatdpltintegration03/delivery-agent:latest
```
...
oci fn application create \
  --compartment-id ocid1.compartment.oc1..aaaaaaaavp2ks4ff7nk3vgm2i6il7sxnh4by474iyeuqsc57z4sogx7nj5sa \
  --display-name delivery-agent-app \
  --subnet-ids '["ocid1.subnet.oc1.iad.aaaaaaaa6wrawaxqs2svulc4spjtmbj3c4yhrajurtq6bxkk4t22khlzotza"]'
```
...
oci fn function create \
  ...
  --image iad.ocir.io/orasenatdpltintegration03/delivery-agent:latest \
  --memory-in-mbs 1024 \
...
oci fn function update \
  ...
  --config OCI_COMPARTMENT_ID=ocid1.compartment.oc1..aaaaaaaavp2ks4ff7nk3vgm2i6il7sxnh4by474iyeuqsc57z4sogx7nj5sa

oci fn function update \
  --function-id <FUNCTION_ID> \
  --config OCI_OS_NAMESPACE=orasenatdpltintegration03

oci fn function update \
  --function-id <FUNCTION_ID> \
  --config OCI_OS_BUCKET=delivery_image_repo
...
3. Image: `iad.ocir.io/orasenatdpltintegration03/delivery-agent:latest`
4. Memory: 1024 MB
